...
class product:
    def __init__(self,root):
        ...
        #create obj ref instance of Database class
        ...
        def insert():
            ...
            if (len(pId.get())!=0):
                ...
                tkinter.messagebox.askyesno("WAREHOUSE INVENTORY SALES PURCHASE MANAGEMENT SYSTEM","Enter Product id")
        ...
        def search():
            ...
            for row in p.search(pId.get(),pName.get(),pQty.get(),pPrice.get(),pCompany.get(),pContact.get()):
                productList.insert(END,row,str(""))         
                productRec()
            print('Product : search method finsihed \n')
        ...
        self.labelpQty.grid(row=3,column=0,stick=W)
        self.txtpQty = Entry(LeftBodyFrame,font=('arial',20,'bold'),textvariable=pQty,width=30)
        
        self.txtpQty.grid(row=3,column=1,stick=W)
        
        self.labelpCompany=Label(LeftBodyFrame,font=('arial',15,'bold'),text="Maufacturing Company :",padx=2,pady=2,bg='white',fg='blue')
        ...
        self.labelpC4.grid(row=9,column=0,stick=W)
        ...
        productList=Listbox(RightBodyFrame,width=40,height=16,font=('arial',14,'bold'),
                yscrollcommand=scroll.set)
        
        #called above created productRec
...
class Database:
    ...
    def conn(self):
        ...
        cur=con.cursor()
        query="create table if not exists product(pid integer primarykey,pname text,price text,qty text,company text, contact text)"
